...
/*
// Always rotate along the body axis to get the correct result.
// Compare the final pose w.r.t world frame
TEST_CASE_METHOD (ECM, __FILE__"_CheckRotation", "") 
{
  {
    // Exercise 2.10 from Spong Text book. 
    Eigen::Affine3d W_r_x_phi(Eigen::AngleAxisd(-M_PI_2, Vector3d::UnitX()));
    Eigen::Affine3d B_r_z_theeta(Eigen::AngleAxisd(M_PI, Vector3d::UnitZ()));
    Eigen::Affine3d W_r_y_zhi(Eigen::AngleAxisd(M_PI_2, Vector3d::UnitY()));

    Matrix3d finalPose = W_r_y_zhi.rotation() * W_r_x_phi.rotation() * B_r_z_theeta.rotation();
    std::cout << std::endl << "2.10" << std::endl << finalPose << std::endl;
  }

  {
    // Exercise 2.11 from Spong Text book
    Eigen::Affine3d W_r_x_phi(Eigen::AngleAxisd(-M_PI_2, Vector3d::UnitX()));
    Eigen::Affine3d W_r_z_theeta(Eigen::AngleAxisd(M_PI_2, Vector3d::UnitZ()));
    Eigen::Affine3d B_r_x_zhi(Eigen::AngleAxisd(M_PI, Vector3d::UnitX()));

    Matrix3d finalPose = W_r_z_theeta.rotation() * W_r_x_phi.rotation() * B_r_x_zhi.rotation();
    std::cout << std::endl << "2.11" << std::endl << finalPose << std::endl;
  }

  {
    // Exercise 2.13 from Spong Text book
    Eigen::Affine3d W_r_x_phi(Eigen::AngleAxisd(-M_PI_2, Vector3d::UnitX()));
    Eigen::Affine3d W_r_z_theeta(Eigen::AngleAxisd(M_PI_2, Vector3d::UnitZ()));
    Eigen::Affine3d B_r_x_zhi(Eigen::AngleAxisd(M_PI, Vector3d::UnitX()));
    Eigen::Affine3d W_r_z_alpha(Eigen::AngleAxisd(-M_PI_2, Vector3d::UnitZ()));

    Matrix3d finalPose = W_r_z_alpha.rotation() * W_r_z_theeta.rotation() * 
      W_r_x_phi.rotation() * B_r_x_zhi.rotation();
    std::cout << std::endl << "2.13" << std::endl << finalPose << std::endl;
  }
}
*/
